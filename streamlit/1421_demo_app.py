...
def generate_response_with_locations(query, results, query_locations):
    ...
    # Location summary
    if query_locations:
        response += "### 🌍 Locations Identified in Query:\n"
        for loc in query_locations[:5]:
            confidence_emoji = "🟢" if loc.get('confidence') == 'high' else "🟡" if loc.get(
    ...
    if results:
        ...
        if high_relevance:
            ...
            for i, result in enumerate(high_relevance[:3], 1):
                ...
                response += f"   📊 Confidence: {result['similarity']:.1%} | 📍 Source: {result['database']}\n\n"

        if medium_relevance:
            response += "#### ⚡ Medium Relevance:\n"
    ...
    if any(word in query_lower for word in ['zheng he', 'treasure fleet', 'ming fleet']):
        response += """
        **Zheng He (1371–1433)** was a Chinese mariner, explorer, diplomat, and fleet admiral during the Ming dynasty.
...
def main():
    ...
    # Header
    st.markdown('<h1 class="main-header">🗺️ 1421 Geographical Research AI</h1>', unsafe_allow_html=True)
    ...
    # Sidebar
    with st.sidebar:
        st.header("🗄️ Research Databases")
        ...
        if not databases:
            ...
            if st.checkbox("Enable Test Mode", value=True):
                ...
                st.success("✅ Test mode enabled - using sample data")
        else:
            st.success(f"✅ Loaded {len(databases)} database(s)")
            for db_name in databases.keys():
                doc_count = len(databases[db_name]['documents'])
                st.info(f"📁 {db_name} ({doc_count:,} documents)")
    ...
    # Main interface
    st.header("🔍 Ask a Geographical Question")
    ...
    with col3:
        map_clicked = st.button("🗺️ Generate Map", type="primary", use_container_width=True)

    # Process query
    if (search_clicked or map_clicked) and query:
        with st.spinner("🔍 Searching documents and extracting locations..."):
    ...
    if 'current_query' in st.session_state:
        ...
        with col_response:
            ...
            with st.expander("📋 Detailed Location Data", expanded=False):
                if query_locations:
        ...
        with col_map:
            ...
            if query_locations or document_locations:
                ...
                if fig:
                    ...
                    with st.expander("🌋 Additional Map Views", expanded=False):
                        # Density map
                    ...
                    with col_export1:
                        if st.button("📥 Export Map Data", use_container_width=True):
                            map_data_export = {
                                'query': query,
                    ...
                    with col_export2:
                        if st.button("🗺️ Save Map Image", use_container_width=True):
        ...
        if results:
            ...
            for i, (tab, result) in enumerate(zip(tabs, results[:8])):
                with tab:
                    ...
                    if locations_in_doc:
                        st.markdown("**📍 Locations mentioned in this document:**")
