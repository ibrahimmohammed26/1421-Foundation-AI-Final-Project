...
/* Fullscreen map button - kept for functionality but hidden checkbox removed */
.fullscreen-map-btn {
    background: #d4af37;
    color: #000;
    border: none;
                ...
                for session in st.session_state.chat_sessions:
                    if session['id'] == st.session_state.current_chat_id:
                        ...
                        if len(session['history']) == 1:
                            session['name'] = question_input[:30] + ('...' if len(question_input) > 30 else '')
                        break
...
def show_map_page(system):
    ...
    # Animation controls - FIXED to work properly
    ...
    if st.session_state.animation_playing:
        if st.session_state.current_year < 1421:
            ...
            time.sleep(0.8)  # 0.8 seconds per year
            st.rerun()
        else:
            st.session_state.animation_playing = False
