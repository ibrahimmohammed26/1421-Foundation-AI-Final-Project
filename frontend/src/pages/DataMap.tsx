...
const EXCLUDE_PATTERNS = ["gallery", "minoan", "atlantis", "fresco", "artist impression"];

// Score a document's relevance to a search point
function scoreDoc(doc: any, point: DataPoint): number {
  const title    = (doc.title || "").toLowerCase();
  const preview  = (doc.content_preview || "").toLowerCase();
  const name     = point.name.toLowerCase();
  const termParts = point.searchTerm.toLowerCase().split(" ");
  let score = 0;

  // Strong match: location name in title
  if (title.includes(name)) score += 20;
  // Medium: search term words in title
  for (const word of termParts) {
    if (word.length > 3 && title.includes(word)) score += 8;
  }
  // Weaker: location name in preview
  if (preview.includes(name)) score += 6;
  for (const word of termParts) {
    if (word.length > 3 && preview.includes(word)) score += 2;
  }
  // Backend similarity score bonus
  if (doc.similarity_score != null) score += doc.similarity_score * 5;
...
const ALL_DATA_POINTS: DataPoint[] = [
  ...
  { id: "sumatra",    name: "Sumatra",    lat: -0.59,  lon: 101.34, year: 1406, category: "voyage",
    ...
    searchTerm: "Sumatra Palembang Zheng He" },
  ...
  { id: "brunei",     name: "Brunei",     lat:  4.94,  lon: 114.95, year: 1408, category: "evidence",
    event: "Chinese porcelain and artefacts found indicating Ming dynasty trade.",
  ...
  { id: "calicut",    name: "Calicut",    lat: 11.26,  lon:  75.78, year: 1407, category: "voyage",
    ...
    searchTerm: "Calicut Kozhikode Malabar Zheng He" },
  ...
  { id: "maldives",   name: "Maldives",   lat:  3.20,  lon:  73.22, year: 1413, category: "evidence",
    event: "Chinese vessels recorded visiting the Maldives; artefacts found.",
    searchTerm: "Maldives Chinese" },

  // ── MIDDLE EAST ──────────────────────────────────────────────────────
  { id: "hormuz",     name: "Hormuz",     lat: 27.16,  lon:  56.28, year: 1414, category: "voyage",
    event: "Voyage 4 — Persian Gulf reached; 18 states sent tribute to China.",
  ...
  { id: "venice",     name: "Venice",     lat: 45.44,  lon:  12.33, year: 1428, category: "evidence",
    event: "Fra Mauro's 1450 map shows detailed knowledge of Africa and Asia — possibly derived from Chinese sources.",
  ...
  { id: "broome",     name: "Broome",     lat: -17.96, lon: 122.23, year: 1421, category: "evidence",
    event: "Beeswax figures and Chinese coins discovered on Australia's northwest coast.",
  ...
  { id: "sydney",     name: "Sydney",     lat: -33.87, lon: 151.21, year: 1421, category: "evidence",
    event: "Claims of Chinese presence in eastern Australia in 1421 Foundation research.",
  ...
  { id: "south-island-nz",name: "South Island, NZ", lat: -44.00, lon: 170.50, year: 1421, category: "evidence",
    event: "Genetic and archaeological evidence of Chinese contact with New Zealand before European arrival.",
    searchTerm: "New Zealand Maori Chinese genetics" },

  // ── SOUTH AMERICA ────────────────────────────────────────────────────
  { id: "ecuador",    name: "Ecuador",    lat: -1.83,  lon: -78.18, year: 1421, category: "evidence",
    event: "Chinese chickens (pre-Columbian) and sweet potato evidence points to Pacific contact.",
  ...
  { id: "brazil",     name: "Brazil",     lat: -14.24, lon: -51.93, year: 1421, category: "evidence",
    event: "1421 Foundation research cites possible Chinese presence on the Brazilian coast.",
...
export default function DataMap() {
  ...
  const fetchRelatedDocs = async (point: DataPoint) => {
    ...
    try {
      ...
      for (const term of [point.searchTerm, point.name, `${point.name} Zheng He`]) {
        try {
      ...
      // Score each doc for relevance to this point
      const scored: RelatedDoc[] = filtered.map((doc) => ({
        id:              doc.id,
        title:           doc.title,
        ...
        url:             doc.url,
        _relevanceScore: scoreDoc(doc, point),
      }));
      ...
      // Only keep docs that have some relevance (score > 0)
      const relevant = unique.filter((d) => d._relevanceScore > 0);

      setRelatedDocs(relevant.slice(0, 8));
    } catch {
      setRelatedDocs([]);
    } finally {
          ...
          <div className="w-80 bg-white border-l border-gray-200 flex flex-col z-[999] shadow-lg">
            ...
            <div className="px-4 py-2 border-b border-gray-100">
              <p className="text-xs font-semibold text-gray-600 flex items-center gap-1.5">
                ...
                Related documents — ranked by relevance
              </p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
              ...
              {!docsLoading && relatedDocs.length === 0 && (
                <div className="text-center py-6">
                  <p className="text-xs text-gray-400 mb-3">No relevant documents found for "{selectedPoint.name}".</p>
                  <button
              ...
              {!docsLoading && relatedDocs.length > 0 && (
                <p className="text-xs text-gray-400">Most relevant first:</p>
              )}
              {!docsLoading && relatedDocs.map((doc, index) => (
                <div key={doc.id}
                  ...
                  <div className="flex items-start gap-2">
                    {/* Uniform gold badge — number = rank */}
