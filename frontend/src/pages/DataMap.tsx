...
function scoreDoc(doc: any, point: DataPoint): number {
  ...
  const hasRequired = point.requiredKeywords.some((kw) => {
    return combined.includes(kw.toLowerCase());
  });
...
// All data points, and there is a lot
...
export default function DataMap() {
  ...
  // Filter visible points based on category and search
  const visiblePoints = useMemo(() => {
    const searchQuery = locationSearch.trim().toLowerCase();
    
    return ALL_DATA_POINTS.filter((point) => {
      // Filter by category
      ...
      // Filter by search
      if (searchQuery) {
        return point.name.toLowerCase().includes(searchQuery) ||
               point.event.toLowerCase().includes(searchQuery);
      }
  ...
  const handleLocationInput = (value: string) => {
    ...
    if (value.trim().length > 0) {
      ...
      const matches = ALL_DATA_POINTS.filter((point) => {
  ...
  const fetchRelatedDocs = async (point: DataPoint) => {
    ...
    try {
      ...
      for (const term of point.searchTerms) {
        try {
          const response = await searchDocuments(term, 20);
          ...
          // Skip failed searches for individual terms
        }
      }
      ...
      for (const doc of filtered) {
        const score = scoreDoc(doc, point);
        if (score > 0) {
          scored.push({
            id: doc.id,
            title: doc.title,
    ...
    <div className="flex flex-col h-full bg-gray-100">
      ...
      <div className="bg-white border-b border-gray-200 flex-shrink-0">
              ...
              <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-xl z-[2000] overflow-hidden">
                {suggestions.map((point) => (
                  ...
                  >
                    ...
                    {point.year && (
                      <span className="text-gray-400 text-xs flex-shrink-0">{point.year}</span>
                    )}
                  </button>
                ))}
              </div>
        ...
        <div className="px-6 pb-2.5 flex items-center gap-3 flex-wrap">
          ...
          {/* Voyage button */}
          <button
            onClick={() => setFilterCategory("voyage")}
            className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border transition-colors ${
          ...
          {/* Evidence button */}
          <button
            onClick={() => setFilterCategory("evidence")}
            className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border transition-colors ${
